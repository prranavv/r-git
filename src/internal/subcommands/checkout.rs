use crate::{Result, internal::{commit_hash::CommitHash, utils::{checkout_branch, checkout_commit_hash}}};
use std::{fs, path::PathBuf};
use std::io;
use std::io::Write;
use crate::RGitError;

pub fn checkout(commit_hash:&CommitHash)->Result<()>{
    let files = fs::read_dir("./.rgit/refs/heads")?;
    for file in files{
        let file = file?;
        let file_name = file.file_name().into_string().unwrap();
        if file_name == commit_hash.to_string(){
            return checkout_branch(&commit_hash.to_string());
        }
    }
    ...
    fs::write(format!(".rgit/HEAD"), format!("{}",commit_hash.to_string()))
